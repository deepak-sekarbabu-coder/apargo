...
import { validateFirebaseConfig, getFirebaseSetupGuide } from '@/lib/firebase-config-validator';
import { healthMonitor, type ConnectionHealth } from '@/lib/firebase-health-monitor';
...
export function NotificationSystemTest() {
  ...
  const runTests = async () => {
    ...
    try {
      ...
      if (configValidation.isValid) {
        tests[0] = { 
          ...
          details: configValidation.warnings.length > 0 ? `Warnings: ${configValidation.warnings.join(', ')}` : undefined
        };
      } else {
        tests[0] = { 
          name: 'Firebase Configuration', 
    ...
    if (user?.apartment) {
      try {
        const notificationsRef = collection(db, 'notifications');
        const apartmentQuery = query(notificationsRef, where('toApartmentId', '==', user.apartment));
                ...
                {notifications.length > 0 ? (
                  <div className="space-y-2">
                    {notifications.map((notification) => (
                      <div key={notification.id} className="p-2 bg-gray-50 dark:bg-gray-800 rounded text-sm">
