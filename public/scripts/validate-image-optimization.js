...
function validateImageOptimization() {
    ...
    console.log(`   ✓ Size parameter (=s64-c): ${results.details.filter(d => d.optimized).length}/${results.details.length}`);
    console.log(`   ✓ Responsive srcSet: ${results.details.filter(d => d.srcSet).length}/${results.details.length}`);
    console.log(`   ✓ Sizes attribute: ${results.details.filter(d => d.sizes).length}/${results.details.length}`);
    console.log(`   ✓ Lazy loading: ${results.details.filter(d => d.lazy).length}/${results.details.length}`);
...
function compareImageSizes() {
    ...
    const googleImages = Array.from(document.querySelectorAll('img'))
