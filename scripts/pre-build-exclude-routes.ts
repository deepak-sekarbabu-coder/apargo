...
const EXCLUDED_NESTED_ROUTES: string[] = [
...
function moveRoutesToTemp(): void {
    ...
    EXCLUDED_ROUTES.forEach(route => {
        ...
        if (fs.existsSync(sourcePath)) {
            try {
                ...
                console.warn(`   ⚠️  Could not exclude /api/${route}:`, errorMessage);
            }
        }
    });
    ...
    EXCLUDED_NESTED_ROUTES.forEach(nestedRoute => {
        ...
        if (fs.existsSync(nestedSourcePath)) {
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : String(error);
                console.warn(`   ⚠️  Could not exclude /api/${nestedRoute}:`, errorMessage);
            }
        }
    });
...
function restoreRoutes(): void {
    ...
    EXCLUDED_ROUTES.forEach(route => {
        ...
        if (fs.existsSync(sourcePath)) {
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : String(error);
                console.warn(`   ⚠️  Could not restore /api/${route}:`, errorMessage);
            }
        }
    });
    ...
    EXCLUDED_NESTED_ROUTES.forEach(nestedRoute => {
        ...
        if (fs.existsSync(nestedSourcePath)) {
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : String(error);
                console.warn(`   ⚠️  Could not restore /api/${nestedRoute}:`, errorMessage);
            }
        }
    });
    ...
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        console.warn('   ⚠️  Could not clean up temp directory:', errorMessage);
    }
}
